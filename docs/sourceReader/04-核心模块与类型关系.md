...
| 方法 | 位置 | 作用 |
|---|---|---|
| `run_conversation()` | `run_agent.py:8597` | 转发到真实循环 |
| `chat()` | `run_agent.py:9077` | 简单接口，返回最终文本 |
| `_build_system_prompt()` | `run_agent.py:4895` | 转发到 `agent/system_prompt.py` |
| `_execute_tool_calls()` | `run_agent.py:8428` | 工具调用执行入口 |
| `_persist_session()` | `run_agent.py:2020` | 持久化会话 |
| `_invoke_tool()` | `run_agent.py:8507` | 单工具调用转发 |
...
| 方法 | 位置 | 作用 |
|---|---|---|
| `create_session()` | `hermes_state.py:6110` | 创建会话 |
| `get_session()` | `hermes_state.py:9509` | 读取会话 |
| `append_message()` | `hermes_state.py:11059` | 追加消息 |
| `append_messages_batch()` | `hermes_state.py:11205` | 批量追加 |
| `get_messages()` | `hermes_state.py:11961` | 读取消息 |
| `get_messages_around()` | `hermes_state.py:12135` | 上下文窗口读取 |
| `get_messages_as_conversation()` | `hermes_state.py:12309` | 转成对话格式 |
