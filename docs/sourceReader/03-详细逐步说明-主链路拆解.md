...
- `agent/conversation_loop.py:_restore_or_build_system_prompt()`，约在 `734` 行；
- `run_agent.py:AIAgent._build_system_prompt()`，约在 `4741` 行；
...
- `model_tools.py:handle_function_call()`，约在 `1192` 行；
- `tools/registry.py:ToolRegistry.register()`，约在 `737` 行。
...
- `hermes_state.py:SessionDB.append_message()`，约在 `9267` 行；
- `hermes_state.py:SessionDB.get_messages()`，约在 `10138` 行。
