...
| 错误表面呈现 | `agent/error_surface.py` | 错误向用户/前端的呈现（本轮新增） |
| 会话修复 | `agent/transcript_repair.py` | 会话转录修复（本轮新增） |
...
| 尾部脚手架清理 | `run_agent.py` | `AIAgent._drop_trailing_empty_response_scaffolding()` @2060 |
| 工具错误截断 | `tools/registry.py` | `_MAX_TOOL_ERROR_CHARS` @33 |
| 持久化 | `run_agent.py` | `AIAgent._persist_session()` @2020 |
