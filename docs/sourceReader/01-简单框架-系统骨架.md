...
| 边界 | 真实文件 | 真实符号 |
|---|---|---|
| CLI 进程入口 | `hermes_cli/main.py` | `main()` @13103 |
| CLI 子命令入口 | `hermes_cli/main.py` | `cmd_chat(args)` @3125 |
| 顶层 parser | `hermes_cli/_parser.py` | `build_top_level_parser()` @136 |
| REPL 编排器 | `cli.py` | `HermesCLI` @5049 |
| Agent 门面 | `run_agent.py` | `AIAgent` @422 |
| Agent 门面转发 | `run_agent.py` | `AIAgent.run_conversation()` @8597 |
| 真实对话循环 | `agent/conversation_loop.py` | `run_conversation()` @1834 |
| 每轮上下文装配 | `agent/turn_context.py` | `build_turn_context()` @508 |
| 工具分发 | `model_tools.py` | `handle_function_call()` @1240 |
| 工具注册表 | `tools/registry.py` | `register()` @763 |
| 会话存储 | `hermes_state.py` | `SessionDB` @4385 |
| 配置加载 | `hermes_cli/config.py` | `load_config()` @3570 |
| profile 路径 | `hermes_constants.py` | `get_hermes_home()` @114 |
| 日志初始化 | `hermes_logging.py` | `setup_logging()` @259 |
| 网关运行时 | `gateway/run.py` | `GatewayRunner` @6950 |
| 网关同步入口 | `gateway/run.py` | `run_sync()` @5455 |
...
| 模块 | 一句话职责 | 关键入口符号 |
|---|---|---|
| `hermes_cli/main.py` | CLI 进程入口、argparse、子命令分发、启动自愈 | `main()` @13103 |
| `hermes_cli/_parser.py` | 顶层 parser 与 chat subparser 构建 | `build_top_level_parser()` |
| `cli.py` | REPL 编排器：输入、命令、皮肤、Agent 装配 | `HermesCLI` @5049、`process_command()` @12118、`load_cli_config()` @412 |
| `hermes_cli/cli_agent_setup_mixin.py` | `HermesCLI` 的 Agent 装配逻辑 | `_init_agent()` @339 |
| `run_agent.py` | AIAgent 门面：参数、预算、中断、转发到循环 | `AIAgent` @422、`run_conversation()` @8597、`chat()` @9077 |
| `agent/conversation_loop.py` | 真实同步对话循环（LLM 调用 + 工具执行 + 预算/中断） | `run_conversation()` @1834 |
| `model_tools.py` | 工具发现（`get_tool_definitions`）与分发（`handle_function_call`） | `handle_function_call()` @1240 |
| `tools/registry.py` | 中央注册表：schema、handler、toolset、check_fn | `register()` @763 |
| `tools/*.py` | 137 个工具实现，import 时自注册 | — |
| `toolsets.py` | toolset 定义与 `_HERMES_CORE_TOOLS` | — |
| `hermes_state.py` | 会话 SQLite 库（FTS5 搜索、schema、portability） | `SessionDB` @4385 |
| `hermes_cli/config.py` | config.yaml + .env 加载 | `load_config()` @3570 |
...
| `gateway/run.py` | 网关运行时：平台事件 → turn | `GatewayRunner` @6950、`run_sync()` @5455 |
...
这解释了为什么 `tools/` 里有 137 个文件却只有一部分进 `_HERMES_CORE_TOOLS`：很多是 `check_fn` 门控（如 Home Assistant、memory provider），默认不随每次 API 调用发送。
